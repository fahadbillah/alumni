 ...
 */
 ...
 .controller('ListCtrl',['$http', '$scope', '$cookies', '$rootScope', '$location', 'USER_ROLES', 'AUTH_EVENTS', 'AuthService', function ($http, $scope, $cookies, $rootScope, $location, USER_ROLES, AUTH_EVENTS, AuthService) {
 	...
 	$scope.getAlumni = function(limit,offset) {

 		$scope.limit = limit;
 		$scope.offset = offset;

 		$http.get('api/index.php/user/get_alumni_list/'+$scope.limit+'/'+$scope.offset)
 	...
 	$scope.getAlumni($scope.limit,$scope.offset);
 	...
 	$scope.exportList = function() {
 		$http.get('api/index.php/user/export_alumni_list')
 		.then(function(data) {
 			console.log(data);
