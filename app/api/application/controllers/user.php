...
class User extends CI_Controller {
	...
	{
		if (isset($this->session->userdata('user_data')['role']) && $this->session->userdata('user_data')['role'] !== 'admin') {
			$returned_data['success'] = false;
			$returned_data['message'] = 'Not Authorized';
			jsonify($returned_data);
		}
		$returned_data['total_count'] = $this->user_model->get_total_alumni_count();
		$returned_data['jobs'] = $this->user_model->get_alumni_single_answer('jobTitle', $limit);
		$returned_data['list'] = $this->user_model->get_alumni_list($limit,$offset);

		foreach ($returned_data['list'] as $key => $value) {
			foreach ($returned_data['jobs'] as $k => $v) {
				if ($v['user_id'] == $value['user_id']) {
					$returned_data['list'][$key]['jobTitle'] = $v['question_answer'];
				}
			}
		}
	...
	public function export_alumni_list()
	{
		if (($this->session->userdata('user_data')['role'] !== 'admin')) {
			...
			jsonify($returned_data);
		}else if((int) $this->session->userdata('user_data')['id'] !== 1){
			$returned_data = array(
				'success' => false,
				'message' => 'Not authorized'
				);
		...
		// $returned_data['total_count'] = 10;
		$returned_data['total_count'] = $this->user_model->get_total_alumni_count();
		$returned_data['jobs'] = $this->user_model->get_alumni_single_answer('jobTitle', $returned_data['total_count']);
		$returned_data['list'] = $this->user_model->get_alumni_list_export($returned_data['total_count'],0);
		...
		$fp = fopen('./uploads/alumnilist.csv', 'w+');

		fputcsv($fp, array('User Id','First Name','Last Name','Phone','Email','NSU ID','Join Date','Designation'));
		foreach ($returned_data['list'] as $fields) {
			fputcsv($fp, $fields);
		}
		$read = base64_encode(file_get_contents('./uploads/alumnilist.csv'));
		fclose($fp);
		unlink('./uploads/alumnilist.csv');
// pshakur@gmail.com
		...
		$msg = array(
			...
			"from_name" => "SBEAlumni",
			"subject" => "Alumni list excel file export!",
			"to" => array(array("email" => 'pshakur@gmail.com')),
			"track_opens" => true,
			"track_clicks" => true,
			"auto_text" => true,
			'attachments' => array(
 ...
 public function reply_to_feedback()
 {
 	...
 	$message .= '<br>';
 	// $message .= '\r\n';
 	$message .= '<strong>Original Message</strong>';
 	$message .= '<br>';
 	$message .= '<br>';
 	...
 	$this->email->from('alumni.association@nsubusinessalumni.org', 'NSU Business Alumni');
 	$this->email->to($data['receiver'],$data['name']);
 	$this->email->subject('Feedback Reply');
 ...
 {
 	// $apikey = $this->load->config('mandrillcon');
 	$this->load->library('mandrill', 'Ux-hKu4KpAS6w0EOJUkg5g'); //load mandrill and provide apikey

 	$params = array(
 		"html" => "<p>\r\n\tHi Adam,</p>\r\n<p>\r\n\tThanks for <a    href=\"http://mandrill.com\">registering</a>.</p>\r\n<p>etc etc</p>",
 		"text" => null,
 		"from_email" => "no-reply@nsubusinessalumni.org",
 		"from_name" => "nsubusinessalumni",
 	...
 	vd($this->mandrill->messages->send($params, true));
 	// try {
 	// 	echo $result['wow'];
 	// } catch (Exception $e) {
